...
function getMatchedTab(location, children) {
  ...
  return tabs.find(t => !!matchPath(location.pathname, { path: t.tab.props.path }));
}
...
function getStyles(matchedTab) {
  ...
  const element = React.isValidElement(matchedTab.tab.props.component) ?
    k(matchedTab.tab.props.component, { ...matchedTab.tab.props, ...matchedTab.tab.props.component.props }) :
...
function TabbedPage({ children, header, className, onChange, caret }) {
  ...
  useEffect(() => {
    ...
    const dir = (previous && previous.matchedTab) ?
      previous.matchedTab.index > matchedTab.index ? "r2l" : "l2r"
      : "l2r";
    setDir(dir);
  ...
  const tabContents = uiAnimations ? animatedStyles(styles, dir) : staticStyles();
