...
const Bar = ({
  ...
  let bottomSidebar = null;
  function onBottomSidebarClick() {
    bottomSidebar.style.display = (bottomSidebar.style.display === "block") ? "none" : "block";
  }
  return (
    <div className={"sidebar-menu " + (isTestNet ? "sidebar-testnet" : "sidebar-mainnet")}>
      ...
      {(gettingStarted || errorPage) ? null : (
        <div>
          ...
          <div className="sidebar-menu-total-balance-extended" ref={(el) => { bottomSidebar = el; }} style={{display: isShowingAccounts ? "block" : "none"}}>
