          ...
          {unmined !== null && unmined !== undefined && unmined.length > 0 ?
            unmined.map(function(tx) {
              ...
              if ( totalFundsReceived + totalChange + fee < totalDebit) {
                ...
                return (<TxRow key={tx.getHash()} tx={tx} direction={'out'} pending txAmount={txAmount} txDescription={txDescription} />);
              } else {
                txDescription = {direction:'Received at:',addressStr: receiveAddressStr};
                txAmount = totalFundsReceived;
                return (<TxRow key={tx.getHash()} tx={tx} direction={'in'} pending txAmount={txAmount} txDescription={txDescription} />);
              }
            })
            : <p></p>
          }
          ...
          {mined !== null && mined !== undefined && mined.length > 0 ?
            mined.map(function(txInfo) {
              ...
              if ( totalFundsReceived + totalChange + fee < totalDebit) {
                txDescription = {direction:'Sent', addressStr: ''};
                txAmount = totalDebit - fee - totalChange - totalFundsReceived;
                return (<TxRow key={tx.getHash()} tx={tx} direction={'out'} showTxDetail={showTxDetail} txAmount={txAmount} txDescription={txDescription} date={date}/>);
              } else {
                txDescription = {direction:'Received at:',addressStr: receiveAddressStr};
                txAmount = totalFundsReceived;
                return (<TxRow key={tx.getHash()} tx={tx} direction={'in'} showTxDetail={showTxDetail} txAmount={txAmount} txDescription={txDescription} date={date}/>);
              }
            }) :
          <div></div>
        }
