...
export const expandSideBar = () => (dispatch, getState) => {
  if (!sel.expandSideBar(getState())) {
    dispatch({ type: EXPAND_SIDE_MENU });
  }
};

export const reduceSideBar = () => (dispatch, getState) => {
  if (sel.expandSideBar(getState())) {
    dispatch({ type: REDUCE_SIDE_MENU });
  }
};

export const sidebarToBottom = () => (dispatch, getState) => {
  if (!sel.sidebarOnBottom(getState())) {
    dispatch({ type: SIDEBAR_TO_BOTTOM });
  }
};

export const onSidebarLeaveBottom = () => (dispatch, getState) => {
  if (sel.sidebarOnBottom(getState())) {
