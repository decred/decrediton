...
export const rescanAttempt = (beginHeight) => (dispatch, getState) => {
  dispatch({ type: RESCAN_ATTEMPT });
  return wallet.rescan(sel.walletService(getState()), beginHeight, rescanResponse => {
    console.log("Rescanned thru", rescanResponse.getRescannedThrough());
    dispatch({ rescanResponse, type: RESCAN_PROGRESS });
  })
    .then(() => {
...
const importScriptSuccess = (importScriptResponse, votingAddress, cb) => (dispatch) => {
...
export const constructTransactionAttempt = (account, confirmations, outputs, all) =>
  (dispatch, getState) => {
    account = parseInt(account);
    confirmations = parseInt(confirmations);
    const walletService = sel.walletService(getState());
    const promise = all
      ? wallet.constructTransaction(walletService, account, confirmations, outputs)
      : wallet.constructSendAllTransaction(walletService, account, confirmations, outputs);
    return promise.then(res => dispatch({ constructTxResponse: res, type: CONSTRUCTTX_SUCCESS }))
      .catch(error => dispatch({ error, type: CONSTRUCTTX_FAILED }));
  };
