    ...
    .then((resp) => {
      ...
      const doReloadTickets = getStakeInfoResponse
        ? checkedFields.reduce(
            (a, v) => a || getStakeInfoResponse[v]() !== resp[v](),
            false
          )
        : false;
...
function filterTickets(tickets, filter) {
  return tickets.filter((v) =>
    filter.status.length ? filter.status.indexOf(v.status) > -1 : true
  );
}
...
// to make.
const getTicketsFromTransactions = async (
  walletService,
  startIdx,
  endIdx,
  maxCount,
    ...
    for (let i = 0; i < txs.length; i++) {
      ...
      if (txs[i].type !== TransactionDetails.TransactionType.TICKET_PURCHASE) {
        ...
        const decodedSpender = wallet.decodeRawTransaction(
          Buffer.from(txs[i].tx.getTransaction())
        );
        const spenderInputs = decodedSpender.inputs;
        txHash = reverseRawHash(
      ...
      hashes.map((h) =>
        (async () => {
          try {
            ...
            const ticketWasSpent =
              ["voted", "revoked"].indexOf(ticket.status) > -1;
...
export const getTickets = () => async (dispatch, getState) => {
  ...
  if (ticketsFilter.listDirection === "desc") {
    startRequestHeight = getTicketsStartRequestHeight
      ? getTicketsStartRequestHeight
      : currentBlockHeight;
    endRequestHeight = 1;
    desc = true;
  ...
  if (desc) {
    ...
    const ticketsMap = minedTickets.reduce((m, t) => {
      m[t.txHash] = t;
      return m;
    }, {});
    ...
    const ticketsMap = filtered.reduce((m, t) => {
      m[t.txHash] = t;
      return m;
    }, {});
    newMinedTickets.unshift(
      ...minedTickets.filter((t) => !ticketsMap[t.txHash])
    );
  }
...
export const getTransactions = () => async (dispatch, getState) => {
  ...
  while (
    !noMoreTransactions &&
    filtered.length < maximumTransactionCount &&
    !reachedGenesis
  ) {
    ...
    try {
      ...
      lastTransaction = mined.length
        ? mined[mined.length - 1]
        : lastTransaction;
      reachedGenesis =
        transactionsFilter.listDirection === "desc" &&
        lastTransaction &&
        lastTransaction.height === 1;
      ...
      for (let i = 0; i < newFiltered.length; i++) {
        ...
        if (stakeTypes.indexOf(tx.type) > -1) {
          ...
          // the overview.
          tx.outputs = await getNonWalletOutputs(
            decodeMessageService,
            walletService,
            tx
          );
  ...
  if (
    transactionsFilter.types.indexOf(
      TransactionDetails.TransactionType.REGULAR
    ) > -1
  ) {
    recentRegularTransactions = [...unminedTransactions, ...minedTransactions];
    recentRegularTransactions = recentRegularTransactions.slice(
      0,
      recentTransactionCount
    );
  } else if (
    transactionsFilter.types.indexOf(TransactionDetails.TransactionType.VOTE) >
    -1
  ) {
    recentStakeTransactions = [...unminedTransactions, ...minedTransactions];
    recentStakeTransactions = recentStakeTransactions.slice(
      0,
      recentTransactionCount
    );
  }
...
export const newTransactionsReceived = (
  ...
  await Promise.all(recentRegularTransactions).then(
    (r) => (recentRegularTransactions = r)
  );
