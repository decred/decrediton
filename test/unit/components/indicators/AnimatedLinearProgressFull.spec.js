...
test("tests default AnimatedLinearProgressFull", () => {
  ...
  const checkLinearProgressBoxes = (expectedWidth, expectedBoxCount) => {
    ...
    expect(linearProgressBar).toHaveClass("linear-progress-bar");
  ...
  expect(screen.getByText(testProps.text)).toHaveClass(
    `linear-progress-text ${testProps.animationType}`
  );
  ...
  ).toHaveClass("linear-progress-bar error");
...
test("tests when deamon is synced", () => {
  ...
  expect(textElement.closest("div.linear-progress")).toHaveClass(
    `linear-progress ${testProps.animationType}`
  );
  expect(textElement.previousSibling).toHaveStyle("");
  expect(textElement.previousSibling).toHaveClass("linear-progress-bar");
