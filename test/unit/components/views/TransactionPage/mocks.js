...
mockRegularTransactionList.forEach((tx) => {
  tx.blockHash = bufferToHex(tx.blockHash);
...
mockStakeTransactionList.forEach((tx) => {
  tx.blockHash = bufferToHex(tx.blockHash);
  if (tx.ticket) {
    tx.ticket.blockHash = bufferToHex(tx.ticket.blockHash);
  }
  if (tx.spender) {
    tx.spender.blockHash = bufferToHex(tx.spender.blockHash);
  }
